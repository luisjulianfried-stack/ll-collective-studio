...
const links = [['Explore', '#work'], ['Services', '#services'], ['Growth', '#growth'], ['About', '#about'], ['Process', '#process'], ['Contact', '#contact']];
...
export function Navigation() {
  ...
  return <>
    ...
    {open && <div className="menu-overlay" ref={dialog} role="dialog" aria-modal="true" aria-label="Hauptmenü"><div className="menu-top"><Link href="/" className="brand" onClick={() => setOpen(false)}><BrandMark/><span className="brand-name">COLLECTIVE<br/>STUDIO</span></Link><button className="menu-trigger" aria-label="Menü schließen" onClick={() => setOpen(false)}>CLOSE <X size={18} strokeWidth={1.5}/></button></div><div className="menu-content"><p className="eyebrow">EXPLORE THE STUDIO / 2026</p><nav aria-label="Hauptnavigation">{links.map(([label, href], i) => <Link key={href} href={`/${href}`} onClick={() => setOpen(false)} className="menu-link" style={{ animationDelay: `${i * 65}ms` }}><span>{String(i + 1).padStart(2, '0')}</span>{label}<ArrowUpRight size={24} strokeWidth={1}/></Link>)}</nav></div><div className="menu-bottom"><Link href="/#contact" onClick={() => setOpen(false)}>KONTAKT AUFNEHMEN ↗</Link><div>{site.instagram && <a href={site.instagram} target="_blank" rel="noreferrer">INSTAGRAM</a>}{site.linkedin && <a href={site.linkedin} target="_blank" rel="noreferrer">LINKEDIN</a>}<Link href="/impressum" onClick={() => setOpen(false)}>IMPRESSUM</Link><Link href="/datenschutz" onClick={() => setOpen(false)}>DATENSCHUTZ</Link></div></div></div>}
  </>;
}
