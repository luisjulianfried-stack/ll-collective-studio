...
export function ProjectGallery() {
  const track = useRef<HTMLDivElement>(null); const [active, setActive] = useState(0); const [atEnd, setAtEnd] = useState(false);
  ...
  const scroll = () => { const t = track.current; if (!t) return; const cards = [...t.children] as HTMLElement[]; setAtEnd(t.scrollLeft >= t.scrollWidth - t.clientWidth - 4); setActive(cards.reduce((best, card, i) => Math.abs(card.offsetLeft - t.offsetLeft - t.scrollLeft) < Math.abs(cards[best].offsetLeft - t.offsetLeft - t.scrollLeft) ? i : best, 0)); };
  return <section id="work" className="work-section section-pad work-v2">
    ...
    {total > 1 && <div className={`gallery-controls${total === 2 ? ' gallery-controls-duo' : ''}`}><span className="gallery-hint">← DRAG / SWIPE →</span><div className="gallery-bar"><span style={{ transform: `scaleX(${atEnd ? 1 : (active + 1) / total})` }}/></div><span className="counter">{pad(atEnd ? total : active + 1)} / {pad(total)}</span><button aria-label="Vorheriges Projekt" onClick={() => go(active - 1)} disabled={active === 0}><ArrowLeft size={20}/></button><button aria-label="Nächstes Projekt" onClick={() => go(active + 1)} disabled={atEnd || active === total - 1}><ArrowRight size={20}/></button></div>}
  </section>;
}
